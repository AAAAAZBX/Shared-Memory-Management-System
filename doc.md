...
**当前状态**：内存池固定为 100MB（25,600 × 4KB 块）

**计划扩展**：
- 将内存池扩展到 1GB（256,000 × 4KB 块）
...
**空间占用**：
- 内存池：100MB（当前），计划扩展至 1GB
- 元数据：~100KB（取决于描述长度），扩展后约 1MB
- 位图：~3.2KB（当前），扩展后约 32KB
- 内存映射表：O(m)，m 为内存块数
- 总计：~100.1MB（当前），扩展后约 1GB

### 常见问题

**Q: 为什么内存池大小是 100MB？**  
A: 当前为演示版本，可根据需求修改 `kPoolSize` 常量。计划扩展至 1GB。
