...
- 🚧 **开发中**：TCP 客户端接口（通过 TCP 服务器提供客户端访问接口）
...
- **协议设计**：
  - ALLOC: 数据格式为 `description\0content`，服务器自动生成 Memory ID
  - UPDATE/READ/DELETE: 数据格式为 `memory_id\0[content]`（UPDATE 需要 content）
  - STATUS: 返回所有内存块的状态信息

**当前状态**：
- TCP 服务器基础框架已实现（`tcp_server.h/cpp`）
- 网络协议定义已完成（`protocol.h/cpp`）
- **待更新**：TCP 服务器实现需要更新为 Memory ID 系统（当前仍使用 userID）
- **待集成**：TCP 服务器需要集成到主程序（`main.cpp`）
