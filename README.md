...
- [核心特性](#核心特性)
  ...
  - [已实现功能](#已实现功能)
    ...
    - [2. 内存分配（`alloc`）](#2-内存分配alloc)
    - [3. 内存释放（`free` / `delete`）](#3-内存释放free--delete)
    - [4. 内容更新（`update`）](#4-内容更新update)
    - [5. 内存紧凑（`compact`）](#5-内存紧凑compact)
    - [6. 状态查询（`status`）](#6-状态查询status)
    - [7. 内容读取（`read`）](#7-内容读取read)
    - [8. 命令行界面](#8-命令行界面)
    - [9. 数据持久化](#9-数据持久化)
    - [10. 批量执行（`exec`）](#10-批量执行exec)
    - [11. 内存池重置（`reset`）](#11-内存池重置reset)
    - [12. TCP 客户端接口 ✅](#12-tcp-客户端接口-)
    - [13. 文件上传功能 ✅](#13-文件上传功能-)
    - [14. UTF-8 中文支持 ✅](#14-utf-8-中文支持-)
- [技术架构](#技术架构)
  - [核心类设计](#核心类设计)
