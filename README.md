...
本项目是一个基于 C++17 实现的共享内存管理系统，采用固定块大小（4KB）的内存池管理策略，提供高效的内存分配、释放和紧凑功能。系统通过 TCP 服务器提供客户端接口，支持多客户端通过网络连接访问共享内存池。客户端程序使用 Python 实现，无需安装额外依赖。
...
- 支持多客户端并发连接
- 提供 Python 客户端程序（`client/client.py`）
...
- ✅ **客户端实现指南**：提供通用的客户端实现文档和 Python 参考实现
...
2. **实现客户端**
   ...
   客户端可以使用任何支持 TCP Socket 的编程语言实现（C、C++、Python、Java、Go 等）。
...
with MemoryClient("127.0.0.1", 8888) as client:
    memory_id = client.alloc("My Data", "Hello, World!")
    client.read(memory_id)
    client.update(memory_id, "New content")
    client.status()
```
