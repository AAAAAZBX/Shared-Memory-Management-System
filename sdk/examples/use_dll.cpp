// 使用 DLL 的示例程序
// 演示如何直接链接 DLL 并使用共享内存管理功能

#include "../include/smm_api.h"
#include <iostream>
#include <cstring>

int main() {
    std::cout << "=== Shared Memory Management DLL 示例 ===\n\n";

    // 1. 创建内存池（直接调用 DLL，无需网络连接）
    std::cout << "创建内存池...\n";
    SMM_PoolHandle pool = smm_create_pool(1024 * 1024 * 1024); // 1GB
    if (!pool) {
        std::cerr << "创建内存池失败: " 
                  << smm_get_error_string(smm_get_last_error()) << "\n";
        return 1;
    }
    std::cout << "内存池创建成功\n\n";
    ...
    // 7. 保存到文件
    std::cout << "保存内存池状态...\n";
    err = smm_save(pool, "memory_pool.dat");
    if (err == SMM_SUCCESS) {
        std::cout << "保存成功\n\n";
    } else {
        std::cerr << "保存失败: " << smm_get_error_string(err) << "\n";
    }
